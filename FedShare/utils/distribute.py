...
def uniform_distribute_sleep(dataset, args):
    globally_shared_data_idx = []
    idxs = np.arange(len(dataset.train_label))
    labels = np.array(dataset.train_label.numpy())
    idxs_labels = np.vstack((idxs, labels))
    idxs_labels = idxs_labels[:,idxs_labels[1,:].argsort()]
    idxs = idxs_labels[0]
    labels = idxs_labels[1]
...
def train_dg_split_sleep(dataset, args):
    dg_idx = []
    train_idx = []
    idxs = np.arange(len(dataset.train_label))
    labels = np.array(dataset.train_label.numpy())
