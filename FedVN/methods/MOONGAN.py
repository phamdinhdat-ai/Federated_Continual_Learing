...
def train_MOONGAN(data_obj, act_prob, learning_rate, batch_size, epoch,
    ...
    if trial or (not os.path.exists('%sModel/%s/%s/%dcom_sel.pt' % (data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            for clnt in selected_clnts:
                ...
                cur_model.load_state_dict(copy.deepcopy(dict(avg_model_ft.state_dict())))
                if clnt_state_buffer[clnt]==0:
                    old_models = []
                else:
                    old_net = model_func().to(device)
                    set_client_from_params(old_net, clnt_params_list[clnt])
                    old_models = [old_net]
                # print(old_models)
                ...
                for old_net in old_models:
                    ...
                    old_net.eval()
                    # old_net.to('cpu')
                for params in avg_model_ft.parameters():
                    params.requires_grad = False
                avg_model_ft.eval()
                # avg.model.to('cpu')

                clnt_models[clnt] = train_model_moon(cur_model, old_models, avg_model_ft, trn_x, trn_y,
                                                tst_x, tst_y, mu, tau,
