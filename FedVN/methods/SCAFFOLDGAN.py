...
def train_SCAFFOLDGAN(data_obj, act_prob, learning_rate, batch_size, n_minibatch,
    ...
    if trial or (not os.path.exists('%sModel/%s/%s/%dcom_sel.pt' % (data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            avg_model_ft = set_client_from_params(model_func().to(device), avg_model_params)
