...
def train_FedDynGAN(data_obj, act_prob,
    ...
    ft_cld_mdls = list(range(n_save_instances))  # Finetune models
    ...
    trn_ft_cld_perf = np.zeros((com_amount, 2))
    tst_ft_cld_perf = np.zeros((com_amount, 2))
        ...
        for i in range(com_amount):
            if os.path.exists('%sModel/%s/%s/ins_avg_%dcom.pt'
                ...
                if os.path.exists(
                    ...
                    trn_ft_cld_perf[:i + 1] = np.load(
                        '%sModel/%s/%s/%d_com_trn_ft_cld_perf.npy' % (data_path, data_obj.name, suffix, (i + 1)))
                    tst_ft_cld_perf[:i + 1] = np.load(
                        '%sModel/%s/%s/%d_com_tst_ft_cld_perf.npy' % (data_path, data_obj.name, suffix, (i + 1)))
    ...
    if trial or (
        ...
        if saved_itr == -1:
            ...
            ft_cld_model = model_func().to(device)
            ft_cld_model.load_state_dict(copy.deepcopy(dict(init_model.state_dict())))

            cld_mdl_param = get_mdl_params([cur_cld_model], n_par)[0]
            ft_mdl_param = get_mdl_params([ft_cld_model], n_par)[0]
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            ft_mdl_param_tensor = torch.tensor(ft_mdl_param[idx_nonbn], dtype=torch.float32, device=device)
            ...
            for clnt in selected_clnts:
                ...
                model.load_state_dict(copy.deepcopy(dict(ft_cld_model.state_dict())))
                ...
                clnt_models[clnt] = train_model_alg(model, model_func, alpha_coef_adpt,
                                                    ft_mdl_param_tensor, hist_params_diffs_curr,
                ...
                hist_params_diffs[clnt] += curr_model_par - ft_mdl_param
            ...
            ft_cld_model = set_client_from_params(model_func().to(device), cld_mdl_param)
            ft_cld_model = train_global_model_cgan_bs_weight_iloop_fz_dis(ft_cld_model, init_g_model, model_func(), clnt_params_list[selected_clnts], #weight_list[selected_clnts],
            ...
            ft_mdl_param = get_mdl_params([ft_cld_model], n_par)[0]
            # ft_cld_mdl_param[idx_bn] = avg_mdl_param_sel[idx_bn]
            # ft_cld_model = set_client_from_params(model_func().to(device), ft_cld_mdl_param)
            ...
            loss_tst, acc_tst = get_acc_loss(cent_x, cent_y,
                                             ft_cld_model, data_obj.dataset, 0)
            print("**** Ft cld Communication %3d, Cent Accuracy: %.4f, Loss: %.4f"
                  % (i + 1, acc_tst, loss_tst))
            trn_ft_cld_perf[i] = [loss_tst, acc_tst]
            ...
            writer.add_scalars('Loss/train_wd',
                               {
                                   ...
                                   'Ft cloud':
                                       get_acc_loss(cent_x, cent_y, ft_cld_model, data_obj.dataset, weight_decay)[0]
                               }, i
                               )
            ...
            loss_tst, acc_tst = get_acc_loss(data_obj.tst_x, data_obj.tst_y,
                                             ft_cld_model, data_obj.dataset, 0)
            print("**** Ft cld Communication %3d, Test Accuracy: %.4f, Loss: %.4f"
                  % (i + 1, acc_tst, loss_tst))
            tst_ft_cld_perf[i] = [loss_tst, acc_tst]
            ...
            if (not trial) and ((i + 1) % save_period == 0):
                ...
                torch.save(cur_cld_model.state_dict(), '%sModel/%s/%s/cld_avg_%dcom.pt'
                           % (data_path, data_obj.name, suffix, (i + 1)))
                torch.save(ft_cld_model.state_dict(), '%sModel/%s/%s/ft_cld_%dcom.pt'
                           % (data_path, data_obj.name, suffix, (i + 1)))

                torch.save(init_g_model.state_dict(), '%sModel/%s/%s/generator_%dcom.pt'
                           % (data_path, data_obj.name, suffix, (i + 1)))
                ...
                np.save('%sModel/%s/%s/%d_com_trn_ft_cld_perf.npy' % (data_path, data_obj.name, suffix, (i + 1)),
                        trn_ft_cld_perf[:i + 1])
                np.save('%sModel/%s/%s/%d_com_tst_ft_cld_perf.npy' % (data_path, data_obj.name, suffix, (i + 1)),
                        tst_ft_cld_perf[:i + 1])
                ...
                if (i + 1) > save_period:
                    ...
                    os.remove('%sModel/%s/%s/%d_com_tst_cur_cld_perf.npy' % (
                        data_path, data_obj.name, suffix, i + 1 - save_period))
                    os.remove('%sModel/%s/%s/%d_com_trn_ft_cld_perf.npy' % (
                        data_path, data_obj.name, suffix, i + 1 - save_period))
                    os.remove('%sModel/%s/%s/%d_com_tst_ft_cld_perf.npy' % (
                        data_path, data_obj.name, suffix, i + 1 - save_period))
            ...
            if (i + 1) % save_period == 0:
                ...
                ft_cld_mdls[i // save_period] = ft_cld_model

    return avg_ins_mdls, avg_cld_mdls, avg_all_mdls, ft_cld_mdls, \
           trn_sel_clt_perf, tst_sel_clt_perf, trn_cur_cld_perf, tst_cur_cld_perf, \
           trn_all_clt_perf, tst_all_clt_perf, trn_ft_cld_perf, tst_ft_cld_perf
